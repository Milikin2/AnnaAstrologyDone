(() => {
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        function updateCountdownDisplay(ms) {
            ...
            document.querySelectorAll('[data-timer="hours"]').forEach((el => {
                if (el.childNodes.length > 0) el.childNodes[0].nodeValue = String(hours).padStart(2, "0");
            }));
            document.querySelectorAll('[data-timer="minutes"]').forEach((el => {
                if (el.childNodes.length > 0) el.childNodes[0].nodeValue = String(minutes).padStart(2, "0");
            }));
            document.querySelectorAll('[data-timer="seconds"]').forEach((el => {
                if (el.childNodes.length > 0) el.childNodes[0].nodeValue = String(seconds).padStart(2, "0");
            }));
        }
    ...
    header.addEventListener('click', (e) => {
        ...
        if (e.target.classList.contains('header__nav-link')) {
            ...
            if (targetElement) {
                ...
                currentPos = window.scrollY + rect.top - headerHeight + 1;
            }
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        const titles = document.querySelectorAll(".intro__services-title span");
        const descriptions = document.querySelectorAll(".intro__services-descr span");
        ...
        const linksArray = {
            starter: "./main.html",
            guide: "./match.html",
            roadmap: "./forecast.html",
            horoscope: "https://www.instagram.com/annavonko",
            blueprint: "https://www.instagram.com/annavonko",
            vip: "https://www.instagram.com/annavonko"
        };
        function handleButtonClick(event) {
            const selectedService = event.currentTarget.getAttribute("data-service");
            buttons.forEach((btn => btn.classList.remove("intro__services-button-active")));
            event.currentTarget.classList.add("intro__services-button-active");
            if (parentContainer) parentContainer.setAttribute("data-item", selectedService);
            images.forEach((img => {
                img.style.display = img.getAttribute("data-photo") === selectedService ? "block" : "none";
            }));
            descriptions.forEach((desc => {
                desc.style.display = desc.getAttribute("data-descr") === selectedService ? "block" : "none";
            }));
            if (mainLink && linksArray[selectedService]) mainLink.setAttribute("href", linksArray[selectedService]);
            titles.forEach((item => {
                item.style.display = item.getAttribute("data-title") === selectedService ? "block" : "none";
            }));
            links.forEach((link => {
                link.style.display = link.getAttribute("data-link") === selectedService ? "inline-block" : "none";
            }));
        }
